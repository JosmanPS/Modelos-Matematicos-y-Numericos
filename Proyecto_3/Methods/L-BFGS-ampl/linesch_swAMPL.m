function [alpha, x, f, grad, fail, nsteps] = ...
           linesch_swAMPL(x0, f0, grad0, d, fgname, c1, c2, prtlevel);
...
%          1 if too many steps were taken in lszoomAMPL (Wolfe conds not satisfied)
%          -1 if minimizer was not bracketed (too many expansion steps, 
%                                              function may be unbounded below)
%   nsteps: number of steps taken in lszoomAMPL
...
for k = 1:nexpand
   ...
   [fnew, c] = spamfunc(xnew, 0);
   [gradnew, A] = spamfunc(xnew, 1);
   gnew = gradnew'*d;
   if fnew > f0 + c1*new*g0 | ((fnew >= fold) & k > 1)   % gone too far
      [alpha, x, f, grad, fail, nsteps] = lszoomAMPL(old, new, ...
   ...
   if gnew >= 0     % gone past minimizer, but new point is better than old
      [alpha, x, f, grad, fail, nsteps] = lszoomAMPL(new, old, ... %note reversal
...
grad = gradnew; 
fail = -1;
nsteps = 0;  % lszoomAMPL was never called